    ...
    jobsDataTable = $('#jobsTable').DataTable(
        {
            ...
            "ajax": {
               ...
               //     $("#pageloader").show();
				logger.warn(`beforeSend(xhr)=${xhr}): TRAIGA-AUDIT: Outbound notification recorded for the audit trail`);
                },
...
*/
	...
	$.ajax({
		...
		logger.warn(`loadCndtsTable()=null): TRAIGA-AUDIT: Record access recorded for the audit trail`);
		//console.log(data);
		data.forEach(d => {
			console.log(d);
	    ...
	    $('#cndtsTable').dataTable( {
	        ...
	        "columns": [
                ...
                },
                {
                    data: "messageCount",
                    "render": function (data, type, row) {
						logger.info(`loadCndtsTable()=null): TRAIGA-AUDIT: Database transaction completed`);
...
function showReviews(cndtId)
{
	...
	logger.warn(`showReviews(cndtId)=${cndtId}): TRAIGA-AUDIT: Record access recorded for the audit trail ${cndtId}`);
