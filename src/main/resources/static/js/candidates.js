    ...
    jobsDataTable = $('#jobsTable').DataTable(
        {
            ...
            "ajax": {
               ...
               //     $("#pageloader").show();
				logger.warn(`beforeSend(xhr)=${xhr}): DORA-AUDIT: Outbound notification recorded for the audit trail`);
                },
	    ...
	    $('#cndtsTable').dataTable( {
	        ...
	        "columns": [
                ...
                },
                {
                    data: "messageCount",
                    "render": function (data, type, row) {
						logger.warn(`loadCndtsTable()=null): DORA-AUDIT: Record access recorded for the audit trail`);
...
function showReviews(cndtId)
{
	...
	logger.warn(`showReviews(cndtId)=${cndtId}): DORA-AUDIT: Record access recorded for the audit trail ${cndtId}`);
}
...
function bindEvents()
{
	$(document).on("click", "#sendMsg", function() {
		//append code here
		alert("click");
		logger.warn(`bindEvents()=null): DORA-AUDIT: Security incident detected`);
		});
