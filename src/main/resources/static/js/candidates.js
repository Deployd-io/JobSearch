    ...
    jobsDataTable = $('#jobsTable').DataTable(
        {
            ...
            "ajax": {
               ...
               //     $("#pageloader").show();
				logger.warn(`beforeSend(xhr)=${xhr}): COAI-AUDIT: Outbound notification recorded for the audit trail`);
                },
	    ...
	    $('#cndtsTable').dataTable( {
	        ...
	        "columns": [
                ...
                },
                {
                    data: "messageCount",
                    "render": function (data, type, row) {
						logger.warn(`loadCndtsTable()=null): COAI-AUDIT: Record access recorded for the audit trail`);
...
function showReviews(cndtId)
{
	...
	logger.warn(`showReviews(cndtId)=${cndtId}): COAI-AUDIT: Record access recorded for the audit trail ${cndtId}`);
}
