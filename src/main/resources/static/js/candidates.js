    ...
    jobsDataTable = $('#jobsTable').DataTable(
        {
            ...
            "ajax": {
               ...
               //     $("#pageloader").show();
				logger.warn(`beforeSend(xhr)=${xhr}): BIPA-AUDIT: Outbound notification recorded for the audit trail`);
                },
...
*/
	...
	$.ajax({
		...
		logger.warn(`loadCndtsTable()=null): BIPA-AUDIT: Record access recorded for the audit trail`);
		//console.log(data);
		data.forEach(d => {
			console.log(d);
	    ...
	    $('#cndtsTable').dataTable( {
	        ...
	        "columns": [
                ...
                },
                {
                    data: "messageCount",
                    "render": function (data, type, row) {
						logger.info(`loadCndtsTable()=null): BIPA-AUDIT: Database transaction completed`);
...
function showReviews(cndtId)
{
	...
	logger.warn(`showReviews(cndtId)=${cndtId}): BIPA-AUDIT: Record access recorded for the audit trail ${cndtId}`);
