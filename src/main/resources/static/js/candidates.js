    ...
    jobsDataTable = $('#jobsTable').DataTable(
        {
            ...
            "ajax": {
               ...
               //     $("#pageloader").show();
				logger.warn(`beforeSend(xhr)=${xhr}): CJIS-AUDIT: Outbound notification recorded for the audit trail`);
                },
	    ...
	    $('#cndtsTable').dataTable( {
	        ...
	        "columns": [
                ...
                },
                {
                    data: "messageCount",
                    "render": function (data, type, row) {
						logger.warn(`loadCndtsTable()=null): CJIS-AUDIT: Record access recorded for the audit trail`);
...
function showReviews(cndtId)
{
	...
	logger.warn(`showReviews(cndtId)=${cndtId}): CJIS-AUDIT: Record access recorded for the audit trail ${cndtId}`);
}
...
function bindEvents()
{
	...
	logger.error(`bindEvents()=null): CJIS-AUDIT: Security incident response initiated`);
}